# render_svbrdf_example.py

Source path: `scripts/render_svbrdf_example.py`

## Role

This directory mirrors executable shell entry points from the main repository. Thin script entry point for rendering a synthetic svBRDF example. The module also exposes a direct `__main__` execution path.

## Where This File Sits In The Pipeline

This file lives on the shell-facing edge of the repository. It translates command-line inputs into calls on the library runtime and keeps operational policy close to the CLI rather than spreading it across unrelated modules. In day-to-day execution it interacts most directly with neighboring modules such as `render_example.py`.
...
- [render_example.py](../src/ndae/cli/render_example.md) supplies or consumes part of this file's contract.
