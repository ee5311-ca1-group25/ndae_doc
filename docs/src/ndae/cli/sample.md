...
Source path: `src/ndae/cli/sample.py`

## Role

This file belongs to the `src/ndae/cli` slice of the NDAE repository. Checkpoint sampling CLI.
...
Purpose: Build the sample CLI parser.
...
Signature: `run_sample_cli(argv=None)`

Purpose: Restore a checkpoint and export a PNG sequence.

Expected inputs and outputs:
- The callable boundary is `run_sample_cli(argv=None)`, so the argument order and optional parameters are part of the contract other files depend on.
- Return values follow the local file conventions and are intended to slot into the next stage with minimal extra cleanup.

Step-by-step implementation reading guide:
1. Follow the delegated helpers `Path, build_argparser, build_svbrdf_system, error, from_config, load_config` to see how this symbol sequences lower-level work.
2. Read the return statement last and verify how much normalization, validation, or formatting the function promises its callers.
...
What downstream code assumes:
- Callers rely on this symbol to prepare clean inputs for `Path, build_argparser, build_svbrdf_system, error, from_config` without redoing the same validation or reshaping work.
...
- [__init__.py](../evaluation/__init__.md) supplies or consumes part of this file's contract.
